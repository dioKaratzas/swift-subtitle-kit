...
enum SSACommon {
        ...
        for block in blocks {
            ...
            case "V4 Styles", "V4+ Styles":
                ...
                for line in bodyLines {
                    guard let pair = splitNamedLine(line) else {
                        continue
                    }
            ...
            case "Events":
                ...
                for line in bodyLines {
                    ...
                    guard pair.name.caseInsensitiveCompare("Dialogue") == .orderedSame else {
                        continue
                    }
                    guard !columns.isEmpty else {
                        throw SubtitleError.malformedBlock(
                            format: hintedFormatName,
                            details: "Missing Events format line"
                        )
                    }
                    ...
                    let plainText = StringTransforms.replacing(
                        pattern: #"\\N"#,
                        in: StringTransforms.stripTags(rawText),
                        with: "\n"
                    )
    ...
    private static func splitCommaValues(_ value: String, limit: Int) -> [String] {
        ...
        for _ in 0 ..< (limit - 1) {
            guard let comma = remaining.firstIndex(of: ",") else {
                break
            }
    ...
    private static func scriptInfoMetadata(_ entry: SubtitleEntry) -> [SubtitleAttribute]? {
        guard case let .metadata(meta) = entry,
              ...
              case let .fields(fields) = meta.value else {
            return nil
        }
        return fields
    }
