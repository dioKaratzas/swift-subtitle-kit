...
2. Provide a tag (for example `v1.0.0` or `1.0.0`)
3. Choose target ref (default `main`)
4. Optional: mark as pre-release

The workflow runs tests, creates/pushes the tag, and publishes a GitHub Release
with autogenerated notes.

Release notes are grouped by label categories defined in:

- `.github/release.yml`

Recommended labels on PRs:
...
Labels `skip-changelog` and `no-release-notes` are excluded from notes.
